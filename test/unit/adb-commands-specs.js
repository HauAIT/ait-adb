...
const adb = new ADB({ adbExecTimeout: 60000, udid: 'emulator-5554' });
...
describe('adb commands', withMocks({adb, logcat, ait_process, net}, function (mocks) {
  afterEach(function () {
    mocks.verify();
  });
  ...
  describe('shell', function () {
    ...
    describe('setWifiState', function () {
      it('should call shell with correct args for real device API 29', async function () {
        ...
        mocks.adb.expects('getApiLevel').once().returns(29);
        mocks.adb.expects('shell')
          .once().withExactArgs(['am', 'broadcast', '-a', 'com.ait.armorsettings.wifi',
            '-n', 'com.ait.armorsettings/.receivers.WiFiConnectionSettingReceiver',
            '--es', 'setstatus', 'enable'])
          .returns('');
    ...
    describe('setDataState', function () {
      it('should call shell with correct args for real device API 29', async function () {
        ...
        mocks.adb.expects('requireRunningSettingsApp').once();
        mocks.adb.expects('shell')
          .once().withExactArgs(['am', 'broadcast', '-a', 'com.ait.armorsettings.data_connection',
            '-n', 'com.ait.armorsettings/.receivers.DataConnectionSettingReceiver',
    ...
    describe('setAnimationState', function () {
      const adbArgs = [
        'am', 'broadcast', '-a', 'com.ait.armorsettings.animation',
        '-n', 'com.ait.armorsettings/.receivers.AnimationSettingReceiver',
        '--es', 'setstatus'
      ];
    ...
    describe('setDeviceSysLocaleViaSettingApp', function () {
      it('should call shell with locale settings without script', async function () {
        const adbArgs = ['am', 'broadcast', '-a', 'com.ait.armorsettings.locale',
          '-n', 'com.ait.armorsettings/.receivers.LocaleSettingReceiver',
      ...
      it('should call shell with correct args for real device with Oreo preview device', async function () {
        ...
        mocks.adb.expects('shell')
          .once().withExactArgs([
            ...
            `com.ait.armorsettings/.LocationService`
          ])
          .returns('');
  ...
  describe('app permission', function () {
    ...
    it('should grant requested permission', async function () {
      mocks.adb.expects('shell')
          .once().withArgs(['pm', 'grant', 'com.ait.armorapis', 'android.permission.READ_EXTERNAL_STORAGE']);
      await adb.grantPermission('com.ait.armorapis', 'android.permission.READ_EXTERNAL_STORAGE');
    });
    it('should revoke requested permission', async function () {
      mocks.adb.expects('shell')
          .once().withArgs(['pm', 'revoke', 'com.ait.armorapis', 'android.permission.READ_EXTERNAL_STORAGE']);
      await adb.revokePermission('com.ait.armorapis', 'android.permission.READ_EXTERNAL_STORAGE');
    });
    it('should properly list requested permissions', async function () {
      mocks.adb.expects('shell').once().returns(dumpedOutput);
      const result = await adb.getReqPermissions('com.ait.armor.android');
      for (let perm of [
        'android.permission.ACCESS_NETWORK_STATE',
        'android.permission.WRITE_EXTERNAL_STORAGE',
        'android.permission.INTERNET',
    ...
    it('should properly list granted permissions', async function () {
      mocks.adb.expects('shell').once().returns(dumpedOutput);
      const result = await adb.getGrantedPermissions('com.ait.armor.android');
      for (let perm of [
        'android.permission.MODIFY_AUDIO_SETTINGS',
        'android.permission.RECEIVE_BOOT_COMPLETED',
        'android.permission.BLUETOOTH',
    ...
    it('should properly list denied permissions', async function () {
      mocks.adb.expects('shell').once().returns(dumpedOutput);
      const result = await adb.getDeniedPermissions('com.ait.armor.android');
      for (let perm of [
        'android.permission.MODIFY_AUDIO_SETTINGS',
        'android.permission.RECEIVE_BOOT_COMPLETED',
        'android.permission.BLUETOOTH',
