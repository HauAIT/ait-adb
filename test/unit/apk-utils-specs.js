...
describe('Apk-utils', withMocks({adb, fs, ait_process}, function (mocks) {
  afterEach(function () {
    mocks.verify();
  });
  ...
  describe('cacheApk', function () {
    it('should remove extra apks from the cache', async function () {
      ...
      mocks.adb.expects('shell')
        .once()
        .withExactArgs(['touch', '-am', '/data/local/tmp/armor_cache/1.apk'])
        .returns(B.resolve());
  ...
  describe('installOrUpgrade', function () {
    const pkgId = 'com.ait.armorsettings';
