...
import * as armorSupport from 'armor-support';
...
const keystorePath = path.resolve(__dirname, '..', 'fixtures', 'armortest.keystore');
...
const tempDir = armorSupport.tempDir;
const fs = armorSupport.fs;
...
describe('signing', withMocks({ait_process, helpers, adb, armorSupport, fs, tempDir}, function (mocks) {
  afterEach(function () {
    mocks.verify();
  });
  ...
  describe('signWithCustomCert', function () {
    ...
    it('should fallback to jarsigner if apksigner fails', async function () {
      ...
      if (armorSupport.system.isWindows()) {
      ...
      mocks.ait_process.expects('exec')
        .once().withExactArgs(jarsigner, [
          ...
          { windowsVerbatimArguments: armorSupport.system.isWindows() })
        .returns({});
  ...
  describe.skip('zipAlignApk', function () {
    it('should call exec with correct args', async function () {
      ...
      mocks.tempDir.expects('path')
        .once().withExactArgs({prefix: 'armor', suffix: '.tmp'})
        .returns(alignedApk);
      mocks.adb.expects('initZipAlign')
        .once().withExactArgs()
        .returns('');
      mocks.armorSupport.expects('mkdirp')
