...
import { fs, mkdirp } from 'armor-support';
...
describe('adb commands', function () {
  ...
  it('should be able to turn off animation', async function () {
    ...
    await adb.grantPermission('com.ait.armorsettings', 'android.permission.SET_ANIMATION_SCALE');
  ...
  it('should be able to set device locale via setting app', async function () {
    ...
    // Operation not allowed: java.lang.SecurityException: Package com.ait.armorsettings has not requested permission android.permission.CHANGE_CONFIGURATION
    // is shown if the setting apk is not updated.
    await adb.grantPermission('com.ait.armorsettings', 'android.permission.CHANGE_CONFIGURATION');
  ...
  describe('app permissions', function () {
    before(async function () {
      ...
      let isInstalled = await adb.isAppInstalled('com.ait.armorapis');
      if (isInstalled) {
        await adb.uninstallApk('com.ait.armorapis');
      }
    });
    it('should install and grant all permission', async function () {
      ...
      (await adb.isAppInstalled('com.ait.armorapis')).should.be.true;
      await adb.grantAllPermissions('com.ait.armorapis');
      let requestedPermissions = await adb.getReqPermissions('com.ait.armorapis');
      expect(await adb.getGrantedPermissions('com.ait.armorapis')).to.have.members(requestedPermissions);
    });
    it('should revoke permission', async function () {
      await adb.revokePermission('com.ait.armorapis', 'android.permission.RECEIVE_SMS');
      expect(await adb.getGrantedPermissions('com.ait.armorapis')).to.not.have.members(['android.permission.RECEIVE_SMS']);
    });
    it('should grant permission', async function () {
      await adb.grantPermission('com.ait.armorapis', 'android.permission.RECEIVE_SMS');
      expect(await adb.getGrantedPermissions('com.ait.armorapis')).to.include.members(['android.permission.RECEIVE_SMS']);
    });
  });
  ...
  describe('push file', function () {
    ...
    let localFile = temp.path({prefix: 'armor', suffix: '.tmp'});
    let tempFile = temp.path({prefix: 'armor', suffix: '.tmp'});
