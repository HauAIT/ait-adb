...
import { tempDir, system, mkdirp, fs, util } from 'armor-support';
import { LRUCache } from 'lru-cache';
import {
  getJavaForOs, getApksignerForOs, getJavaHome, APKS_EXTENSION, unsignApk,
} from '../helpers.js';
...
/** @typedef {{output: string, expected: KeystoreHash, keystorePath: string}} SignedAppCacheValue */
/** @type {LRUCache<string, SignedAppCacheValue>} */
...
apkSigningMethods.signWithDefaultCert = async function signWithDefaultCert (apk) {
  ...
  const args = [
    'sign',
    '--key', path.resolve(__dirname, DEFAULT_PRIVATE_KEY),
    '--cert', path.resolve(__dirname, DEFAULT_CERTIFICATE),
    apk,
  ];
  try {
...
apkSigningMethods.zipAlignApk = async function zipAlignApk (apk) {
  ...
  try {
    ...
    throw new Error(`The file at '${apk}' is not writeable. ` +
      ...
      `for the Armor process, so it can zip-align the file`);
  }
  const alignedApk = await tempDir.path({prefix: 'armor', suffix: '.tmp'});
  await mkdirp(path.dirname(alignedApk));
  try {
    await exec(
      (this.binaries).zipalign,
      ['-f', '4', apk, alignedApk]
    );
...
apkSigningMethods.checkApkCert = async function checkApkCert (appPath, pkg, opts = {}) {
  ...
  if (SIGNED_APPS_CACHE.has(appHash)) {
    ...
    const {keystorePath, output, expected} = /** @type {SignedAppCacheValue} */ (SIGNED_APPS_CACHE.get(appHash));
  ...
  try {
    ...
    if (_.includes(errMsg, JAVA_PROPS_INIT_ERROR)) {
      ...
      // Nevertheless, lets make Armor to believe the file is already signed,
