...
import { fs, util, tempDir } from 'armor-support';
import { EOL } from 'os';
import Logcat from '../logcat';
import { sleep, waitForCondition } from 'ait-async';
import { SubProcess, exec } from 'ait-process';
...
methods.grantPermissions = async function grantPermissions (pkg, permissions) {
  ...
  try {
    ...
    const err = /** @type {import('ait-process').ExecError} */(e);
...
methods.killProcessByPID = async function killProcessByPID (pid) {
  ...
  try {
    ...
    try {
      ...
      const err1 = /** @type {import('ait-process').ExecError} */ (e1);
